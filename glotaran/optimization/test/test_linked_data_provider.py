...
from glotaran.optimization.linked_data_provider import LinkedDataProvider
...
def test_no_overlap():
    ...
    data = {
        "dataset1": xr.DataArray(
            np.zeros((3, 2)), coords=[("global", global_axis_1), ("model", model_axis_1)]
        ).to_dataset(name="data"),
    ...
    group = model.get_dataset_groups()["default"]
    scheme = Scheme(model, parameters, data)
    linked_data_provider = LinkedDataProvider(group, scheme)
    #  optimization_group = OptimizationGroup(scheme, model.get_dataset_groups()["default"])
