...
class Parameter(_SupportsArray):
    ...
    label: str = ib(converter=str)
    expression: str | None = ib(None)
    maximum: float = ib(default=np.inf)
    minimum: float = ib(default=-np.inf)
    non_negative: bool = False
    standard_error: float = np.nan
    value: float = ib(default=np.nan)
    vary: bool = ib(default=True)

    transformed_expression: str | None = ib(default=None, init=False, repr=False)

    @label.validator
    def valid_label(self, attribute: Attribute, label: str) -> bool:
        ...
        """
        if (
            VALID_LABEL_REGEX.search(label.replace(".", "_")) is not None
            or label in RESERVED_LABELS
        ):
            raise ValueError(f"'{label}' is not a valid parameter label.")

    @expression.validator
    def valid_expression(self, attribute: Attribute, value: Any):
        if value:
            self.vary = False
            self.transformed_expression = PARAMETER_EXPRESSION_REGEX.sub(
                r"parameters.get('\g<parameter_expression>').value", value
            )

    @value.validator
    def valid_value(self, attribute: Attribute, value: Any):
        if not isinstance(value, float) and value is not np.nan:
            try:
                ...
                raise TypeError(
                    ...
                    f"'{self.label}' has value '{value}' of type '{type(value)}'"
                )

    @maximum.validator
    def valid_max(self, attribute: Attribute, value: Any):
        if not isinstance(value, float) and value is not np.nan:
            try:
                float(value)
            except Exception:
    ...
    @minimum.validator
    def valid_min(self, attribute: Attribute, value: Any):
        if not isinstance(value, float) and value is not np.nan:
            try:
                float(value)
            except Exception:
