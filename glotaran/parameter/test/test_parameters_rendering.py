...
def test_param_group_repr():
    ...
    result_short = Parameters.from_dict({"foo": {"bar": [1, 2, 3]}})
    expected = "Parameters.from_dict({'foo': {'bar': [['1', 1.0], ['2', 2.0], ['3', 3.0]]}})"
...
def test_param_group_repr_from_list():
    ...
    result = Parameters.from_list([["1", 2.3], ["2", 3.0]])
    result_short = Parameters.from_list([2.3, 3.0])
    expected = "Parameters.from_list([['1', 2.3], ['2', 3.0]])"
